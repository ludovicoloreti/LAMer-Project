    ...
    @objc func handleSignUp() {
		...
		Auth.auth().createUser(withEmail: email, password: pass) { (user, error) in
			if error == nil && user != nil {
				...
				guard let uid = user?.uid else { return } // attenzione ai lameronzoli
				print("Utente creato!")
				...
				changeRequest?.commitChanges(completion: { (error) in
					if error == nil {
						print("Nome utente registrato su firebase!")
						...
						dbRef.child("items").child(uid).updateChildValues(userValues, withCompletionBlock: {
							(err, ref) in
							if err == nil {
				...
				print("Errore nella creazione del nuovo utente: \(error!.localizedDescription)")
			}
		}
    }
